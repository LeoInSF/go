...
func main() {
	// NOTE: First we’ll look at basic rate limiting. Suppose we want to limit our handling of incoming requests.
	...
	close(requests)
	// NOTE: This limiter channel will receive a value every 200 milliseconds. This is the regulator in our rate limiting scheme.
	limiter := time.Tick(200 * time.Millisecond)
	// NOTE: By blocking on a receive from the limiter channel before serving each request,
	...
	// NOTE: We may want to allow short bursts of requests in our rate limiting scheme while preserving the overall rate limit.
